...
import io.github.michaelcirkl.ubsa.BlobStorageClientFactory;
...
public final class AsyncProviderFixture implements AutoCloseable {
    ...
    private AsyncProviderFixture(Provider provider, FixtureInitializer initializer) {
        this.provider = provider;
        this.initializer = initializer;
    }

    public static AsyncProviderFixture create(Provider provider) {
    ...
    public AsyncTestContext openContext() {
        return new AsyncTestContext(this);
    }
    ...
    void cleanupBucket(String bucketName) {
        try {
            BlobStorageAsyncClient client = fixture().client;
            PageRequest request = PageRequest.builder().pageSize(100).build();
            while (true) {
                ListingPage<Blob> page = AsyncTestSupport.await(client.listBlobs(bucketName, null, request));
                for (Blob blob : page.getItems()) {
                    AsyncTestSupport.await(client.deleteBlobIfExists(bucketName, blob.getKey()));
                }
                if (!page.hasNextPage()) {
                    break;
                }
    ...
    public void close() {
        ...
        try {
            ...
            throw new IllegalStateException("Failed to close " + provider + " test client.", error);
        }
    }
    ...
    private static AsyncProviderFixture createAwsFixture() {
        return new AsyncProviderFixture(Provider.AWS, () -> {
            ...
            S3AsyncClient nativeClient = builder.build();
            return new InitializedFixture(nativeClient, BlobStorageClientFactory.getAsyncClient(nativeClient));
        });
    }

    private static AsyncProviderFixture createAzureFixture() {
        return new AsyncProviderFixture(Provider.Azure, () -> {
            TestEnvironment env = TestEnvironment.load();
            String connectionString = azureConnectionString(env);
            BlobServiceAsyncClient nativeClient = new BlobServiceClientBuilder()
                    .connectionString(connectionString)
                    .serviceVersion(BlobServiceVersion.V2025_11_05)
                    .buildAsyncClient();
            return new InitializedFixture(() -> { }, BlobStorageClientFactory.getAsyncClient(nativeClient));
        });
    }

    private static AsyncProviderFixture createGcpFixture() {
        return new AsyncProviderFixture(Provider.GCP, () -> {
            TestEnvironment env = TestEnvironment.load();
            String projectId;
            try {
            ...
            Storage nativeClient = StorageOptions.grpc()
    ...
    private static final class InitializedFixture {
        ...
        private InitializedFixture(AutoCloseable nativeClient, BlobStorageAsyncClient client) {
